...
with st.container(border=True):
    #st.image('Track - SPA.png', width=600, use_container_width=True)
    ...
    circuit_choisit = st.selectbox("Choisir un circuit :", ordre)

    filtered = df_lap_by_circuit[df_lap_by_circuit["Track"] == circuit_choisit]
...
with st.container(border=True):
    ...
    # Filtre statique
    df_filtered_gt3 = df_record_data[(df_record_data["Category"] ==  CATEGORIE) & (df_record_data["Circuit"] == circuit_choisit) & (df_record_data["Record"] != "0:00.000")].sort_values("Record")
